    ...
    with tab1:
        ...
        with st.container():
            ...
            st.caption(
                ...
                help="""## How does this work?\nThe search engine is based on Retrieval Augmented Generation (RAG), a technique that tries to enhace the capability of LLMs to retrieve information from unstructured sources. In a first step, we parse all pages of the sustainability statement into machine-readable text using [MistralOCR](https://docs.mistral.ai/capabilities/document/).
                \nWe then embedded this text using Mistral's [embedding model](https://docs.mistral.ai/capabilities/embeddings/) to represent the text in a numerical format. Because of the numerical format, we then find the 10 pages most closely related to the query.
                \nFinally, we use [OpenAI's GPT 4o-mini](https://platform.openai.com/docs/models/gpt-4o-mini) read the user-written prompt and consider the 10 report pages to answer it based on this information.
                \n ### Disclaimer\nThe resulting answer has been generated by an artificial intelligence language model. While we strive for accuracy and quality in our prompt design and with the fact that we only use information from the company itself, please note that the information provided may not be entirely error-free or up-to-date. We recommend independently verifying the content and consulting with professionals for specific advice or information. We do not assume any responsibility or liability for the use or interpretation of this content. This does not constitute investment advice.
                """)
